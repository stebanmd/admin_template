...
namespace Lib.Utils.Extenssions
{
    public static class EnumExtensions
    {
        ...
        public static T FromString<T>(string nome, bool ignoreCase = false) where T : struct, IConvertible
        {
            try
            {
                return (T)Enum.Parse(typeof(T), nome, ignoreCase);
            }
            catch
            {
                return (T)(Enum.GetValues(typeof(T)).GetValue(0));
        ...
        public static bool Exists<T>(string comparar) where T : struct
        {
            try
            {
                ...
                {
                    if (comparar.Equals(item.ToString(), StringComparison.InvariantCultureIgnoreCase))
                    {
                        return true;
                    }
                }
